...
with st.expander("⚙️ Cấu hình API và các tham số", expanded=False):
    ...
    with col1:
        ...
        fail_criterion = st.selectbox(
            "Chọn tiêu chí",
            ["accuracy", "relevance", "completeness", "clarity", "access_control", "average"],
            index=0,
...
with tab1:
    ...
    with col2:
        true_answer = st.text_area("✅ Câu trả lời chuẩn:", height=230, placeholder="Nhập câu trả lời mẫu...")
...
with tab2:
    ...
    if uploaded_file is not None:
        try:
            ...
            st.info("💡 Tip: Bạn có thể click vào ô để chỉnh sửa trực tiếp. Tick ✓ vào cột 'Chọn' để chọn test case muốn chạy.")
            ...
            edited_df = st.data_editor(
                ...
                column_config={
                    ...
                    "Level": st.column_config.SelectboxColumn(
                        ...
                        help="Cấp bậc",
                        width="small",
                        options=["L1", "L2", "L3", "L4", "L5", "L6", "L7", "L8", "L9", "L10", "L11", "L12", "B1", "B2"],
                        required=True
                    ),
                    "Department": st.column_config.SelectboxColumn(
                        ...
                        help="Phòng ban",
                        width="medium",
                        options=["Phòng kinh doanh (Sales)", "Hỗ trợ kinh doanh (Sales Support)", "HR", "Finance"],
                        required=True
                    ),
                },
                key="test_cases_editor_thfc"
            )
            
            # Cập nhật session state
            st.session_state.test_cases_df_thfc = edited_df
            ...
            with col3:
                if st.button("▶️ Chạy test", type="primary", use_container_width=True, key="run_batch_test_thfc"):
                    if len(selected_df) > 0:
                        ...
                        selected_levels = selected_df['Level'].tolist()
                        selected_departments = selected_df['Department'].tolist()
